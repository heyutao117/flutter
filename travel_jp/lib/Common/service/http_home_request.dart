...
class HYHttpHomeRequest {
  static Future<HYHomeDataModelEntity> homeRequest() async {
    const url =
        "/video/zdyhomepage?appname=baispribenlvyou&version=6.7.0&page=0&readtype=1&hardware=android";

    final result = await HYHttpBaseRequest.request(url);
    print(result);
    final dataResult = result["data"];
    HYHomeDataModelEntity homeDataModelEntity =
        HYHomeDataModelEntity.fromJson(dataResult);
  ...
  static Future<HYHomeDataVideoModelEntity> videoRequest(String mainId) async {
    ...
    final String url =
        "/v2/videos?appname=baispribenlvyou&os=ios&version=6.7.0&mainId=$realId&hardware=android";
    final result = await HYHttpBaseRequest.request(url);
    final dataResult = result["data"];
    HYHomeDataVideoModelEntity homeDataVideoModelEntity =
        HYHomeDataVideoModelEntity.fromJson(dataResult);
